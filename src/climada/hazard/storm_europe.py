    ...
    * Synthetic windstorm events (1986 to 2011): https://doi.org/10.24381/cds.ce973f02
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.ssi_full_area = (
            ssi_full_area if ssi_full_area is not None else np.array([], float)
        )

    def read_footprints(self, *args, **kwargs):
        """This function is deprecated, use StormEurope.from_footprints instead."""
        LOGGER.warning(
            "The use of StormEurope.read_footprints is deprecated."
    ...
    def from_footprints(
        ...
        hazard_list = [
            cls._read_one_nc(file_name, centroids, intensity_thres)
    ...
    def from_cosmoe_file(
        ...
        with xr.open_dataset(fp_file) as ncdf:
            ...
            if event_date:
                try:
                    stacked = (
                        ncdf.sel(time=event_date.strftime("%Y-%m-%d"))
                        .groupby("date")
                        .max()
            ...
            haz = cls(
                ...
                date=date,
                event_name=[
                    date_i + "_ens" + str(ens_i)
                    for date_i, ens_i in zip(
                        date_to_str(date), stacked["epsd_1"].values + 1
                    )
                ],
                frequency=np.divide(
                    np.ones_like(event_id), np.unique(ncdf["epsd_1"]).size
                ),
            )
    ...
    def read_icon_grib(self, *args, **kwargs):
        ...
        LOGGER.warning(
            "The use of StormEurope.read_icon_grib is deprecated."
    ...
    def from_icon_grib(
        ...
        if event_date:
            try:
                stacked = (
                    stacked.sel(valid_time=event_date.strftime("%Y-%m-%d"))
                    .groupby("date")
                    .max()
                )
            except KeyError as ker:
        ...
        if description is None:
            description = (
                "icon weather forecast windfield for run started at "
                + run_datetime.strftime("%Y%m%d%H")
            )
        ...
        haz = cls(
            ...
            event_name=[
                date_i + "_ens" + str(ens_i)
                for date_i, ens_i in zip(date_to_str(date), stacked["number"].values)
            ],
            frequency=np.divide(
                np.ones_like(event_id), np.unique(stacked["number"]).size
            ),
        )
        haz.check()
        ...
        for ind_i, file_i in enumerate(file_names):
            gripfile_path_i = Path(file_i[:-4])
            idxfile_path_i = next(
                gripfile_path_i.parent.glob(str(gripfile_path_i.name) + ".*.idx")
            )
        ...
        with xr.open_dataset(file_name) as ncdf:
            ...
            if hasattr(ncdf, "latitude"):
                ...
                raise AttributeError(
                    "netcdf file has no field named latitude or "
    ...
    def _combine_events(self, event_ids):
        ...
        self.intensity = sparse.vstack(
            [self.intensity, sparse.csr_matrix(intensity_tmp)]
        )
        self.event_id = np.append(
            self.event_id[select_other_events], self.event_id.max() + 1
        )
        self.date = np.append(
            self.date[select_other_events], np.round(self.date[select_event_ids].mean())
        )
        ...
        self.frequency = np.append(
            self.frequency[select_other_events], self.frequency[select_event_ids].mean()
        )
        self.orig = np.append(
            self.orig[select_other_events], self.orig[select_event_ids].max()
        )
        if self.ssi_wisc.size > 0:
            self.ssi_wisc = np.append(self.ssi_wisc[select_other_events], np.nan)
        if self.ssi.size > 0:
            self.ssi = np.append(self.ssi[select_other_events], np.nan)
        if self.ssi_full_area.size > 0:
            self.ssi_full_area = np.append(
                self.ssi_full_area[select_other_events], np.nan
            )
        self.check()
    ...
    def plot_ssi(self, full_area=False):
        ...
        axs.scatter(
            ssi_hist["freq_cum"], ssi_hist["ssi"], color="red", label="Historic Events"
        )
        axs.legend()
    ...
    def generate_prob_storms(
        self, reg_id=528, spatial_shift=4, ssi_args=None, **kwargs
    ):
        ...
        if reg_id is not None:
            ...
            sel_cen[spatial_shift:-spatial_shift, spatial_shift:-spatial_shift] = True
        ...
        for index, intensity1d in enumerate(self.intensity):
            ...
            intensity_prob[start:end, :], ssi[start:end] = self._hist2prob(
                intensity1d, sel_cen, spatial_shift, ssi_args, **kwargs
            )
        ...
        new_haz = StormEurope(
            ...
            # years
            frequency=np.divide(
                np.repeat(self.frequency, N_PROB_EVENTS), N_PROB_EVENTS
            ),
            orig=(event_id % 100 == 0),
        )
        new_haz.check()
    ...
    def _hist2prob(
        ...
        # spatial shifts
        # northward
        intensity3d_prob[6:12, :-spatial_shift, :] = intensity3d_prob[
            0:6, spatial_shift:, :
        ]
        # southward
        intensity3d_prob[12:18, spatial_shift:, :] = intensity3d_prob[
            0:6, :-spatial_shift, :
        ]
        # eastward
        intensity3d_prob[18:24, :, spatial_shift:] = intensity3d_prob[
            0:6, :, :-spatial_shift
        ]
        # westward
        intensity3d_prob[24:30, :, :-spatial_shift] = intensity3d_prob[
            0:6, :, spatial_shift:
        ]

        intensity_out = intensity3d_prob.reshape(
            N_PROB_EVENTS, np.prod(self.centroids.shape)
        )
...
def generate_WS_forecast_hazard(
    ...
    if run_datetime is None:
        run_datetime = dt.datetime.today().replace(
            hour=0, minute=0, second=0, microsecond=0
        )
    if event_date is None:
        event_date = dt.datetime.today().replace(
            hour=0, minute=0, second=0, microsecond=0
        ) + dt.timedelta(days=2)
    ...
    if haz_model in ["cosmo1e_file", "cosmo2e_file"]:
        ...
        if haz_file.exists():
            ...
            fp_file = Path(
                str(haz_raw_storage).format(run_datetime.strftime("%y%m%d%H"))
            )
        ...
        raise NotImplementedError(
            "specific 'WS' hazard not implemented yet. "
