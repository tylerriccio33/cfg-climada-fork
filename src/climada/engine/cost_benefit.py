...
class CostBenefit:
    ...
    def __init__(
        ...
        imp_meas_present: Optional[
            Dict[str, Union[float, Tuple[float, float], Impact, ImpactFreqCurve]]
        ] = None,
        imp_meas_future: Optional[
            Dict[str, Union[float, Tuple[float, float], Impact, ImpactFreqCurve]]
        ] = None,
    ):
        """Initilization"""
        ...
        self.imp_meas_future = (
            imp_meas_future if imp_meas_future is not None else dict()
        )
        self.imp_meas_present = (
            imp_meas_present if imp_meas_present is not None else dict()
        )
    ...
    def combine_measures(
        ...
        self._combine_imp_meas(
            new_cb, in_meas_names, new_name, risk_func, when="future"
        )
        if self.imp_meas_present:
            ...
            if imp_time_depen is None:
                imp_time_depen = 1
            self._combine_imp_meas(
                new_cb, in_meas_names, new_name, risk_func, when="present"
            )

        # cost-benefit computation: fill measure's benefit and cost_ben_ratio
        time_dep = new_cb._time_dependency_array(imp_time_depen)
        new_cb._cost_ben_one(
            new_name, new_cb.imp_meas_future[new_name], disc_rates, time_dep
        )
        new_cb._print_results()
        new_cb._print_npv()
        return new_cb

    def apply_risk_transfer(
        self,
        meas_name,
        attachment,
        cover,
        ...
        self.color_rgb[m_transf_name] = np.maximum(
            np.minimum(self.color_rgb[meas_name] - np.ones(3) * 0.2, 1), 0
        )

        _, layer_no = self.imp_meas_future[NO_MEASURE]["impact"].calc_risk_transfer(
            attachment, cover
        )
        layer_no = risk_func(layer_no)

        imp, layer = self.imp_meas_future[meas_name]["impact"].calc_risk_transfer(
            attachment, cover
        )
        ...
        if self.imp_meas_present:
            ...
            _, pres_layer_no = self.imp_meas_present[NO_MEASURE][
                "impact"
            ...
            imp, layer = self.imp_meas_present[meas_name]["impact"].calc_risk_transfer(
                attachment, cover
            )
        ...
        layer_no = disc_rates.net_present_value(
            self.present_year, self.future_year, layer_no
        )
        layer = (
            self.cost_ben_ratio[m_transf_name] * self.benefit[m_transf_name] - cost_fix
        ) / cost_factor
        self._print_results()
    ...
    def plot_cost_benefit(self, cb_list=None, axis=None, **kwargs):
        ...
        if abs(text_pos - self.tot_climate_risk / norm_fact) > 1:
            axis.scatter(
                self.tot_climate_risk / norm_fact, 0, c="r", zorder=200, clip_on=False
            )
            axis.text(
                self.tot_climate_risk / norm_fact,
                0,
                "  Tot risk",
                horizontalalignment="center",
                verticalalignment="bottom",
        ...
        axis.set_ylim(
            ...
            int(
                1
                / np.nanmin(
                    np.ma.masked_equal(np.array(list(self.cost_ben_ratio.values())), 0)
                )
            )
            + 1,
        )
        ...
        x_label = (
            "NPV averted damage over "
            + str(self.future_year - self.present_year + 1)
            + " years ("
            + self.unit
            + " "
            + norm_name
    ...
    def plot_event_view(self, return_per=(10, 25, 100), axis=None, **kwargs):
        ...
        for meas_name, meas_val in self.imp_meas_future.items():
            if meas_name == NO_MEASURE:
                continue
            interp_imp = np.interp(
                return_per, meas_val["efc"].return_per, meas_val["efc"].impact
            )
        ...
        for rp_i, _ in enumerate(return_per):
            ...
            axis.bar(
                rp_i + 1, ref_imp[rp_i], edgecolor="k", fc=(1, 0, 0, 0), zorder=100
            )
    ...
    def plot_waterfall(
        ...
        **kwargs,
        ...
        imp = ImpactCalc(entity.exposures, entity.impact_funcs, hazard).impact(
            assign_centroids=hazard.centr_exp_col not in entity.exposures.gdf
        )
    ...
    def plot_arrow_averted(
        ...
        **kwargs,
        ...
        if accumulate:
            ...
            tot_benefit = np.array(
                [
                    risk_func(self.imp_meas_future[NO_MEASURE]["impact"])
                    - risk_func(self.imp_meas_future[meas]["impact"])
                    for meas in in_meas_names
                ]
            ).sum()
            norm_fact = (
                risk_func(self.imp_meas_future["no measure"]["impact"])
                / bars[3].get_height()
            )
        if combine:
            try:
                LOGGER.info("Combining measures %s", in_meas_names)
                all_meas = self.combine_measures(
                    in_meas_names,
                    "combine",
                ...
                LOGGER.warning(
                    "Use calc() with save_imp=True to get a more accurate "
                    "approximation of total averted damage,"
                )
            if accumulate:
                tot_benefit = all_meas.benefit["combine"]
            else:
                tot_benefit = risk_func(
                    all_meas.imp_meas_future[NO_MEASURE]["impact"]
                ) - risk_func(all_meas.imp_meas_future["combine"]["impact"])

        self._plot_averted_arrow(
            axis,
            bars[3],
            tot_benefit,
    ...
    def plot_waterfall_accumulated(
        ...
        **kwargs,
        ...
        risk_curr = self._npv_unaverted_impact(curr_risk, entity.disc_rates, time_dep)
        LOGGER.info(
            "Current total risk at {:d}: {:.3e}".format(self.future_year, risk_curr)
        )
        ...
        risk_dev = self._npv_unaverted_impact(
            risk_func(imp), entity.disc_rates, time_dep, curr_risk
        )
        LOGGER.info(
            "Total risk with development at {:d}: {:.3e}".format(
                self.future_year, risk_dev
            )
        )

        # socioecon + cc
        risk_tot = self._npv_unaverted_impact(
        ...
        LOGGER.info(
            "Total risk with development and climate change at {:d}: {:.3e}".format(
                self.future_year, risk_tot
            )
        )
        ...
        axis.text(
            1,
            risk_curr / norm_fact,
            str(int(round(risk_curr / norm_fact))),
            horizontalalignment="center",
            verticalalignment="bottom",
            fontsize=12,
            color="k",
        ...
        axis.text(
            2,
            risk_curr / norm_fact + (risk_dev - risk_curr) / norm_fact / 2,
        ...
        axis.text(
            3,
            risk_dev / norm_fact + (risk_tot - risk_dev) / norm_fact / 2,
        ...
        axis.text(
            4,
            risk_tot / norm_fact,
            str(int(round(risk_tot / norm_fact))),
            horizontalalignment="center",
            verticalalignment="bottom",
            fontsize=12,
            color="k",
        ...
        axis.set_title(
            "Total accumulated impact from {:d} to {:d}".format(
                self.present_year, self.future_year
            )
        )
        return axis

    def _calc_impact_measures(
        self,
        hazard,
        exposures,
        meas_set,
        ...
        imp_tmp = ImpactCalc(exposures, imp_fun_set, hazard).impact(
            assign_centroids=False
        )
    ...
    def _calc_cost_benefit(self, disc_rates, imp_time_depen=None):
        ...
        """
        LOGGER.info(
            "Computing cost benefit from years %s to %s.",
            str(self.present_year),
            str(self.future_year),
        )

        if self.future_year - self.present_year + 1 <= 0:
            raise ValueError(
                "Wrong year range: %s - %s."
            ...
            if meas_name == NO_MEASURE:
                ...
                if self.imp_meas_present:
                    self.tot_climate_risk = self._npv_unaverted_impact(
                        ...
                        disc_rates,
                        time_dep,
                        self.imp_meas_present[NO_MEASURE]["risk"],
                    )
                else:
                    self.tot_climate_risk = self._npv_unaverted_impact(
                        self.imp_meas_future[NO_MEASURE]["risk"], disc_rates, time_dep
                    )
                continue
    ...
    def _cost_ben_one(
        self, meas_name, meas_val, disc_rates, time_dep, ini_state=NO_MEASURE
    ):
        ...
        if self.imp_meas_present:
            pres_benefit = (
                self.imp_meas_present[ini_state]["risk"]
        ...
        # discount
        meas_ben = disc_rates.net_present_value(
            self.present_year, self.future_year, meas_ben
        )
        risk_tr = disc_rates.net_present_value(
            self.present_year, self.future_year, risk_tr
        )
        self.benefit[meas_name] = meas_ben
        with np.errstate(divide="ignore"):
            self.cost_ben_ratio[meas_name] = (
                meas_val["cost"][0] + meas_val["cost"][1] * risk_tr
            ) / meas_ben
    ...
    def _time_dependency_array(self, imp_time_depen=None):
        ...
        if imp_time_depen:
            time_dep = (
                np.arange(n_years) ** imp_time_depen / (n_years - 1) ** imp_time_depen
            )
        else:
            time_dep = np.ones(n_years)
        return time_dep

    def _npv_unaverted_impact(
        self, risk_future, disc_rates, time_dep, risk_present=None
    ):
        ...
        if risk_present:
            ...
            tot_climate_risk = disc_rates.net_present_value(
                self.present_year, self.future_year, tot_climate_risk
            )
        else:
            tot_climate_risk = disc_rates.net_present_value(
                self.present_year, self.future_year, time_dep * risk_future
            )
        return tot_climate_risk

    def _combine_imp_meas(
        self, new_cb, in_meas_names, new_name, risk_func, when="future"
    ):
        ...
        sum_ben = np.sum(
            [
                imp_dict[NO_MEASURE]["impact"].at_event
                - imp_dict[name]["impact"].at_event
                for name in in_meas_names
            ],
            axis=0,
        )
        new_imp = copy.deepcopy(imp_dict[in_meas_names[0]]["impact"])
        new_imp.at_event = np.maximum(
            imp_dict[NO_MEASURE]["impact"].at_event - sum_ben, 0
        )
    ...
    def _print_results(self):
        ...
        for meas_name in self.benefit:
            ...
            table.append(
                [
                    meas_name,
                    cost,
                    self.benefit[meas_name] / norm_fact,
        ...
        table = []
        table.append(
            ["Total climate risk:", self.tot_climate_risk / norm_fact, norm_name]
        )
        table.append(
            [
                "Average annual risk:",
                self.imp_meas_future[NO_MEASURE]["risk"] / norm_fact,
                norm_name,
            ]
        )
        table.append(
            [
                "Residual risk:",
                (self.tot_climate_risk - np.array(list(self.benefit.values())).sum())
                / norm_fact,
                norm_name,
            ]
        )
    ...
    def _plot_list_cost_ben(cb_list, axis=None, **kwargs):
        ...
        sort_cb = np.argsort(
            np.array([cb_list[0].cost_ben_ratio[name] for name in m_names])
        )
        xy_lim = [0, 0]
        for i_cb, cb_res in enumerate(cb_list):
            ...
            for meas_id in sort_cb:
                ...
                if i_cb == 0:
                    axis.text(
                        xmin + (cb_res.benefit[meas_n] / norm_fact) / 2,
                        0,
                        "  " + meas_n,
                        horizontalalignment="center",
                        verticalalignment="bottom",
            ...
            xy_lim[0] = max(
                xy_lim[0],
                max(
                    int(cb_res.tot_climate_risk / norm_fact),
            ...
            try:
                with np.errstate(divide="ignore"):
                    xy_lim[1] = max(
                        xy_lim[1],
                        int(1 / cb_res.cost_ben_ratio[m_names[sort_cb[0]]]) + 1,
                    )
